 ...
 */
package com.github.zhycn.dubbo.resteasy.boot;
...
public class ResteasyEmbeddedServletInitializer implements BeanFactoryPostProcessor {
  ...
  // This is how JAXRS_APP_CLASSES_PROPERTY was named originally. It conflicted with
  // "resteasy.jaxrs.app.registration"
  // in case of YAML files, since registration was a child of app from an YAML perspective, which is
  // not allowed.
  // Because of that its name was changed (the ".classes" suffix was added).
  // This legacy property has not been removed though, to keep backward compatibility, but it is
  // marked as deprecated. It will be
  // available only for .properties files, but not for YAML files. It should be finally removed in a
  ...
  private static Logger logger =
      LoggerFactory.getLogger(ResteasyEmbeddedServletInitializer.class);
  ...
  /*
   * Find the JAX-RS application classes. This is done by one of these three options in this order:
   *
   * 1- By having them defined as Spring beans 2- By setting property {@code
   * resteasy.jaxrs.app.classes} via Spring Boot application properties file. This property should
   * contain a comma separated list of JAX-RS sub-classes 3- Via classpath scanning (looking for
   * javax.ws.rs.core.Application sub-classes)
   *
   * First try to find JAX-RS Application sub-classes defined as Spring beans. If that is existent,
   * the search stops, and those are the only JAX-RS applications to be registered. If no JAX-RS
   * application Spring beans are found, then see if Spring Boot property {@code
   * resteasy.jaxrs.app.classes} has been set. If it has, the search stops, and those are the only
   * JAX-RS applications to be registered. If not, then scan the classpath searching for JAX-RS
   * applications.
   *
   * There is a way though to force one of the options above, which is by setting property {@code
   * resteasy.jaxrs.app.registration} via Spring Boot application properties file. The possible
   * valid values are {@code beans}, {@code property}, {@code scanning} or {@code auto}. If this
   * property is not present, the default value is {@code auto}, which means every approach will be
   * tried in the order and way explained earlier.
   *
   * @param beanFactory
   */
  ...
  private void findJaxrsApplications(ConfigurableListableBeanFactory beanFactory) {
    ...
    switch (registration) {
      ...
      default:
        logger.error(
            "JAX-RS application registration method (%s) not known, no application will be registered",
            registration.name());
        break;
    }
  ...
  private JaxrsAppClassesRegistration getJaxrsAppClassesRegistration(
    ...
    String jaxrsAppClassesRegistration =
    ...
    if (jaxrsAppClassesRegistration == null) {
      ...
      logger.info("Property {} has been set to {}", JAXRS_APP_CLASSES_DEFINITION_PROPERTY,
  ...
  private void findJaxrsApplicationBeans(ConfigurableListableBeanFactory beanFactory) {
    ...
    Map<String, Application> applicationBeans =
  ...
  /*
   * Find JAX-RS application classes by scanning the classpath under packages already marked to be
   * scanned by Spring framework
   */
  private void findJaxrsApplicationScanning(BeanFactory beanFactory) {
    ...
    Set<Class<? extends Application>> applications =
  ...
  /*
   * Search for JAX-RS resource and provider Spring beans, which are the ones whose classes are
   * annotated with {@link Path} or {@link Provider} respectively
   *
   * @param beanFactory
   */
  ...
  public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory)
    ...
    for (Class<? extends Application> applicationClass : applications) {
      ...
      if (path == null) {
        logger.warn(
            "JAX-RS Application class {} has no ApplicationPath annotation, so it will not be registered",
